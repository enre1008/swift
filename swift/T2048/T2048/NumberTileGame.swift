...
//  S2048
//
//  Created by Sunny－Joy on 2017/6/28.
...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
    ...
    var scoreView: ScoreViewProtocol?  //显示得分的view
    ...
    func setupSwipeControls() {
        ...
        let downSwipe = UISwipeGestureRecognizer(target: self, action: #selector(NumberTileGameViewController.downCommand(_:))) //后面会有donwCommand方法的定义
    ...
    func followUp() {
        ...
        if userWon {
            ...
            let alertController = UIAlertController(title: "Victory",
                                                    message:"You won!", preferredStyle: .alert)
        ...
        if m.userHasLost() {
            ...
            let alertController = UIAlertController(title: "Defeat",
                                                    message:"You lost...", preferredStyle: .alert)
            let cancelAction = UIAlertAction(title: "Cancel", style: .cancel, handler: nil)
            alertController.addAction(cancelAction)
            self.present(alertController, animated: true, completion: nil)
        }
    ...
    func downCommand(_ r: UIGestureRecognizer!) {
        assert(model != nil)
        let m = model!
        m.queueMove(direction: MoveDirection.down,//手势发生后，controller调用了model的queueMove方法
                                                  //其中MoveDirection是枚举，表示滑动方向
                                                  //进入GameModel.swift的queueMove方法
                    onCompletion: { (changed: Bool) -> () in //本无名闭包将在model调用委托方法之后，执行
                        if changed {
                            self.followUp()
                        }
        })
