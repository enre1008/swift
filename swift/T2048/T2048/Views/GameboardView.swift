...
class GameboardView : UIView {
    ...
    var tiles: Dictionary<IndexPath, TileView> //是以Dictionary的方式存储TileView。
                  //Dictionary中的key，是NSIndexPath，实际运用中将位置坐标x,y（或raw,col）转成NSIndexPath
    ...
    init(dimension d: Int, tileWidth width: CGFloat, tilePadding padding: CGFloat, cornerRadius radius: CGFloat, backgroundColor:
        ...
        cornerRadius = radius
        tiles = Dictionary()
        let sideLength = padding + CGFloat(dimension) * (width + padding) //不理解
    ...
    /// Return whether a gien position is valid. Used for bounds checking.
    ...
    /// Update the gamebaord by moving a single tile from one location to another. If the move is going to collapse two 
    ...
    func moveOneTile(from: (Int,Int), to:(Int,Int), value:Int) {
        ...
        UIView.animate(withDuration: perSquareSlideDuration,
          ...
          /// Update the gameboard by moving two tiles from their original locations to a common destinations. This action always
        ...
        UIView.animate(withDuration: perSquareSlideDuration,
                       ...
                       completion: { finished in
                        ...
                        UIView.animate(withDuration: self.tileMergeExpandTime,
                                       ...
                                       completion: { finished in
                                        // Contract file to original size
                                        UIView.animate(withDuration: self.tileMergeContractTime, animations: {
